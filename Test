...
Content.Size=UDim2.new(1,-(navW+20),1,-56)
	...
	function GUI.notify(title, text, duration, ntype)
		...
		local titleText = tostring(title or "VenturaUI")
		local roughLines = math.max(1, math.ceil(#body / 34))
		local cardH = math.clamp(54 + (roughLines - 1) * 14, 56, 96)

		if #_notifStack >= maxNotifs then
			local oldest = _notifStack[#_notifStack]
			if oldest and oldest.Parent then
				table.remove(_notifStack, #_notifStack)
		...
		card.ZIndex           = 600
		corner(card, 8); stroke(card, Library.Theme.Border)

		local accent = Instance.new("Frame", card)
		accent.Size             = UDim2.new(0, 4, 1, 0)
		accent.BackgroundColor3 = accentCol
		accent.BorderSizePixel  = 0
		accent.ZIndex           = 601; corner(accent, 3)
		...
		closeBtn.FontFace = Font.new("rbxasset://fonts/families/Ubuntu.json", Enum.FontWeight.Bold)
		closeBtn.TextColor3 = Library.Theme.TextDisabled
		closeBtn.ZIndex = 603
		...
		local iconStr = ntype=="success" and "✅ " or ntype=="warning" and "⚠️ " or ntype=="error" and "❌ " or ntype=="info" and "ℹ️ " or ""
		local tTitle = lbl(card, iconStr..titleText, 12, Library.Theme.TextPrimary, Font.new("rbxasset://fonts/families/Ubuntu.json", Enum.FontWeight.Bold))
		tTitle.Size     = UDim2.new(1, -34, 0, 16)
		tTitle.Position = UDim2.new(0, 12, 0, 8)
		tTitle.ZIndex   = 601

		local tText = lbl(card, body, 11, Library.Theme.TextSecondary)
		tText.Size         = UDim2.new(1, -24, 0, cardH - 30)
		tText.Position     = UDim2.new(0, 12, 0, 26)
		tText.ZIndex       = 601
		tText.TextWrapped  = true
		tText.TextTruncate = Enum.TextTruncate.None
		tText.TextYAlignment = Enum.TextYAlignment.Top

		local closing = false
		local function closeCard()
			if closing or not card or not card.Parent then return end
			closing = true
			_removeCard(card)
			tw(card, {
		...
		closeBtn.MouseEnter:Connect(function() tw(closeBtn,{TextColor3=Library.Theme.TextPrimary}) end)
		closeBtn.MouseLeave:Connect(function() tw(closeBtn,{TextColor3=Library.Theme.TextDisabled}) end)
	...
	function GUI:SetAccent(color)
		...
		pcall(function() if _ttFrame then _ttFrame:FindFirstChildOfClass("UIStroke").Color = color end end)
		for _, tab in ipairs(GUI._tabs) do
			if tab._indicator then tab._indicator.BackgroundColor3 = color end
			if tab.Active and tab._navBtn then
				tw(tab._navBtn,{BackgroundColor3=Color3.fromRGB(
					math.clamp(color.R*255*0.18 + Library.Theme.Surface.R*255*0.82,0,255),
					math.clamp(color.G*255*0.18 + Library.Theme.Surface.G*255*0.82,0,255),
					math.clamp(color.B*255*0.18 + Library.Theme.Surface.B*255*0.82,0,255)
				)})
			end
		end
		GUI:SaveConfig()
	...
	function GUI:CreateTab(opts)
		...
		Tab._navBtn.BackgroundColor3=isFirst and Color3.fromRGB(36,36,42) or Library.Theme.Border
		...
		indicator.Size=UDim2.new(0,isFirst and 3 or 0,0.68,0)
		...
		function Tab:Activate()
			...
			tw(Tab._navBtn,{BackgroundTransparency=0.2})
			tw(navText,{TextColor3=Library.Theme.TextPrimary})
			...
			tw(Tab._indicator,{Size=UDim2.new(0,3,0.68,0)})
		end
		function Tab:Deactivate()
			...
			tw(Tab._indicator,{Size=UDim2.new(0,0,0.68,0)})
		end
		...
		local function base(name,h)
			...
			f.Size=UDim2.new(1,0,0,h or 42); corner(f,7); stroke(f,Library.Theme.Border)
			return f
		end

		local _currentSection = nil
		local _sectionConn = Tab._scroll.ChildAdded:Connect(function(child)
			if _currentSection and child:IsA("GuiObject") and child.Parent == Tab._scroll and not child:GetAttribute("VenturaSectionHeader") then
				table.insert(_currentSection._items, child)
				if _currentSection.Collapsed then
					child.Visible = false
				end
			end
		end)
		Tab._scroll.AncestryChanged:Connect(function()
			if not Tab._scroll:IsDescendantOf(game) and _sectionConn then
				_sectionConn:Disconnect()
			end
		end)
		...
		function Tab:Section(opts)
			opts=validate({name="Section",defaultOpen=true},opts)
			local f=Instance.new("Frame",Tab._scroll)
			f.Name="Section"
			f.BackgroundTransparency=1
			f.BorderSizePixel=0
			f.Size=UDim2.new(1,0,0,30)
			f:SetAttribute("VenturaSectionHeader", true)
			...
			local sLbl=lbl(f,opts.name:upper(),12,Library.Theme.TextDisabled,
				Font.new("rbxasset://fonts/families/Ubuntu.json",Enum.FontWeight.Bold))
			sLbl.BackgroundColor3=Library.Theme.Background
			sLbl.BackgroundTransparency=0
			sLbl.Size=UDim2.new(0,#opts.name*8+18,1,0)
			sLbl.ZIndex=2
			...
			arrow.Text=opts.defaultOpen and "−" or "+"
			arrow.TextSize=14
			arrow.FontFace=Font.new("rbxasset://fonts/families/RobotoMono.json",Enum.FontWeight.Bold)
			arrow.TextColor3=Library.Theme.TextDisabled
			arrow.TextXAlignment=Enum.TextXAlignment.Center
			...
			local Section = {Frame=f, Collapsed=not opts.defaultOpen, _items={}}
			_currentSection = Section

			local function apply()
				arrow.Text = Section.Collapsed and "+" or "−"
				for _, item in ipairs(Section._items) do
					if item and item.Parent == Tab._scroll then
						item.Visible = not Section.Collapsed
					end
				end
			end
			...
			hit.MouseEnter:Connect(function()
				tw(sLbl,{TextColor3=Library.Theme.TextSecondary})
				tw(arrow,{TextColor3=Library.Theme.TextSecondary})
			end)
			hit.MouseLeave:Connect(function()
				tw(sLbl,{TextColor3=Library.Theme.TextDisabled})
				tw(arrow,{TextColor3=Library.Theme.TextDisabled})
			end)
		...
		function Tab:Separator		function Tab:Separator()
			local f=Instance.new("Frame",Tab._scroll)
		...
		function Tab:Label(opts)
			...
			local Label={}
			local f=Instance.new("Frame",Tab._scroll)
			f.BackgroundTransparency=1; f.BorderSizePixel=0; f.Size=UDim2.new(1,0,0,30)
			local l2=lbl(f,opts.text,14,opts.color or Library.Theme.TextSecondary)
			pad(l2,0,0,0,6)
			Label.frame=f
			function Label:SetText(v) l2.Text=tostring(v or "") end
			function Label:SetColor(v) if typeof(v)=="Color3" then l2.TextColor3=v end end
			return Label
		end
		...
		function Tab:Button(opts)
			...
			local h=opts.description~="" and 52 or 42
			local Btn={}
			Btn.frame=base("Button",h); pad(Btn.frame,0,10,0,12)
			local bStroke=Btn.frame:FindFirstChildOfClass("UIStroke")
			local nl=lbl(Btn.frame,opts.name,14,Library.Theme.TextPrimary)
			nl.Size=UDim2.new(1,-30,0,18); nl.Position=UDim2.new(0,0,0,h==52 and 8 or 12)
			local dl
			if opts.description~="" then
				dl=lbl(Btn.frame,opts.description,12,Library.Theme.TextSecondary)
				dl.Size=UDim2.new(1,-30,0,14); dl.Position=UDim2.new(0,0,0,29)
			end
			if opts.badge then
				...
				bpf.Size=UDim2.new(0,math.max(#opts.badge*6+10,26),0,16)
				bpf.Position=UDim2.new(1,-18,0.5,0); corner(bpf,4)
				local bpl=Instance.new("TextLabel",bpf)
				bpl.BackgroundTransparency=1; bpl.Size=UDim2.new(1,0,1,0)
				bpl.TextSize=10; bpl.TextXAlignment=Enum.TextXAlignment.Center
			...
			arr.Size=UDim2.new(0,12,0,12); arr.Position=UDim2.new(1,0,0.5,0); arr.Rotation=-90
			...
			function Btn:SetText(v) nl.Text=tostring(v or "") end
			function Btn:SetDescription(v) if dl then dl.Text=tostring(v or "") end end
			return Btn
		end

		function Tab:Toggle		function Tab:Toggle(opts)
			opts=validate({name="Toggle",description="",tooltip="",default=false,callback=function()end},opts)
			local h=opts.description~="" and 52 or 42
			local Toggle={Value=opts.default}
			Toggle.frame=base("Toggle",h); pad(Toggle.frame,0,10,0,12)
			local nl=lbl(Toggle.frame,opts.name,14,Library.Theme.TextPrimary)
			nl.Size=UDim2.new(1,-44,0,18); nl.Position=UDim2.new(0,0,0,h==52 and 8 or 12)
			if opts.description~="" then
				local dl=lbl(Toggle.frame,opts.description,12,Library.Theme.TextSecondary)
				dl.Size=UDim2.new(1,-44,0,14); dl.Position=UDim2.new(0,0,0,30)
			end
			...
			pill.Size=UDim2.new(0,34,0,18); pill.Position=UDim2.new(1,0,0.5,0); corner(pill,9)
			local knob=Instance.new("Frame",pill)
			knob.BackgroundColor3=Color3.fromRGB(215,215,215); knob.BorderSizePixel=0
			knob.AnchorPoint=Vector2.new(0,0.5); knob.Size=UDim2.new(0,14,0,14)
			knob.Position=opts.default and UDim2.new(1,-16,0.5,0) or UDim2.new(0,2,0.5,0)
			corner(knob,7); addTooltip(Toggle.frame,opts.tooltip)
			Toggle.frame.InputBegan:Connect(function(i)
				if i.UserInputType==Enum.UserInputType.MouseButton1
					...
					tw(knob,{Position=Toggle.Value and UDim2.new(1,-16,0.5,0) or UDim2.new(0,2,0.5,0)})
			...
			function Toggle:SetValue(v, silent) Toggle:Set(v, silent) end
			return Toggle
		end
		...
		function Tab:Slider(opts)
			...
			Slider.frame=base("Slider",60); pad(Slider.frame,9,12,10,12)
			local nl=lbl(Slider.frame,opts.name,14,Library.Theme.TextPrimary); nl.Size=UDim2.new(1,-50,0,18)
			local vl=lbl(Slider.frame,tostring(opts.default)..opts.suffix,13,Library.Theme.TextSecondary,nil,Enum.TextXAlignment.Right)
			vl.AnchorPoint=Vector2.new(1,0); vl.Size=UDim2.new(0,48,0,18); vl.Position=UDim2.new(1,0,0,0)
			local track=Instance.new("Frame",Slider.frame)
			track.BackgroundColor3=Color3.fromRGB(13,13,13); track.BorderSizePixel=0
			track.AnchorPoint=Vector2.new(0,1); track.Size=UDim2.new(1,0,0,6)
			...
			thumb.AnchorPoint=Vector2.new(0.5,0.5); thumb.Size=UDim2.new(0,14,0,14)
			thumb.Position=UDim2.new((opts.default-opts.min)/(opts.max-opts.min),0,0.5,0); corner(thumb,7)
			...
			function Slider:SetValue(v, silent) Slider:Set(v, silent) end
			return Slider
		end
		...
		function Tab:Dropdown(opts)
			...
			DD.frame=base("Dropdown",44); DD.frame.ClipsDescendants=true
			...
			local selLbl=lbl(header,opts.default or opts.name,14,Library.Theme.TextPrimary)
			...
			arrowImg.Size=UDim2.new(0,14,0,14); arrowImg.Position=UDim2.new(1,-10,0.5,0); arrowImg.ZIndex=3
			...
			optHolder.Size=UDim2.new(1,0,0,0); optHolder.Position=UDim2.new(0,0,0,40)
			...
			local function refresh()
				...
				for _,item in ipairs(opts.items) do
					...
					opt.TextSize=13; opt.TextXAlignment=Enum.TextXAlignment.Left
					...
					opt.Size=UDim2.new(1,0,0,30); opt.Text=item; opt.ZIndex=5
			...
			function DD:SetValue(v, silent) DD:Set(v, silent) end
			return DD
		end
