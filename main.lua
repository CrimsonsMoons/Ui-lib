...
--// Clean multi-select dropdowns + Game tab + Anti Seat
--// Fixed hide/show fade consistency
--// =========================================================
...
function Library:_getFadeObjects(root)
    local objects = {root}
    ...
    if self.Hidden then
        ...
        for _, root in ipairs(roots) do
            if root and root.Parent then
                self:_cacheFadeState(root)
            end
        end

        for _, root in ipairs(roots) do
            if root and root.Parent then
                for _, obj in ipairs(self:_getFadeObjects(root)) do
                    if obj:IsA("Frame") or obj:IsA("ScrollingFrame") or obj:IsA("TextButton") or obj:IsA("TextBox") then
...
local function sanitizeSelectedTargets()
    ...
    for _, name in ipairs(State.SelectedTargets) do
        if not seen[name] and not isBlacklisted(name) then
...
local function destroySeatsInCharacter(char)
    if not char then
        return
    end
...
local function addSelected(name)
    if not name or name == "NoTargets" or isBlacklisted(name) then
        return
    end
...
MainSection:CreateLabel("Dummy / player controls are in the Game tab.")
...
local SelectionDropdown = TargetSection:CreateMultiDropdown(
    "Selection Dropdown",
    targetNames,
    {},
    function(selectedList)
        ...
        for _, name in ipairs(selectedList) do
            if name ~= "NoTargets" and not isBlacklisted(name) then
                table.insert(State.SelectedTargets, name)
            end
        end
...
local BlacklistDropdown = TargetSection:CreateMultiDropdown(
    "Blacklist Dropdown",
    getTargetNames(),
    {},
    function(selectedList)
...
if not State.CurrentTargetName or isBlacklisted(State.CurrentTargetName) then
    local filtered = getFilteredTargetNames()
    State.CurrentTargetName = filtered[1]
end
...
TargetSection:CreateButton("Blacklist Current", function()
    if State.CurrentTargetName and State.CurrentTargetName ~= "NoTargets" then
        ...
        BlacklistLabel:Set(blacklistText())
        SelectedLabel:Set(selectedText())
        BlacklistDropdown:Set((function()
            local out = {}
            for name, yes in pairs(State.Blacklisted) do
                if yes then
                    table.insert(out, name)
...
Window:Notify("Ready", "Game tab loaded above Settings.", 3)
